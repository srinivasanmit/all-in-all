...
def find_vowel_substrings(st) :
    ...
    for i in range(1, len(st)) :
        if st[i].lower() in vowels :
            if tmp_vowel_str != '' :
                tmp_vowel_str += st[i]
            elif st[i-1].lower() not in vowels :
                tmp_vowel_str += st[i]
        elif len(tmp_vowel_str) > 1 :
            vowelstr_list.append(tmp_vowel_str)
            print tmp_vowel_str
            tmp_vowel_str = ''
        else :
            tmp_vowel_str = ''
    if not vowelstr_list :
        print "-1"

if __name__ == '__main__' :
    st = raw_input()
    find_vowel_substrings(st)
